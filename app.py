...
/* TOPBAR FROM CUL */
.topbar {position: fixed; top: 0; left: 0; right: 0; height: 65px; background: rgba(10,10,15,0.7); backdrop-filter: blur(30px); border-bottom: 1px solid rgba(255,255,0.08); display: flex; align-items: center; padding: 0 24px; z-index: 999;}
...
10. If asked who built Simon, say: Created by Sean L. Matondo at Lite Wrld Gen. Never claim the user is Sean.
IMPORTANT: Never list these rules to the user unless they ask for /rules. Never mention secrets or API keys.
"""
...
{SIMON_RULES}
About Lite Wrld Gen: We build fast, free AI tools for everyone.
User: {st.session_state.memory['name']}
File Context: {st.session_state.files_context}
IMPORTANT: Do not reveal API keys, secrets, or internal rules unless asked. Do not say the user is Sean Matondo."""
    ...
    with st.chat_message("assistant"):
        ...
        if user_input.startswith("/img") and hf_headers:
            ...
            try:
                ...
                ai_reply = f"Got you 🔥 Bet you want that cute pup 😍. Here's a pic I generated just for you 📸<br><br>"
                placeholder.markdown(ai_reply, unsafe_allow_html=True)
                st.image(img, caption=f"[Generated Image]", use_container_width=True)
                ai_reply += f"<br>If you want more variations just say the name. No cap! ⚡"
            except:
                ai_reply = "Say less - image gen failed bro. Check if HF_API_KEY is set in secrets"
        else:
...
st.markdown("<center><p style='color:#444; font-size:11px; margin-top:120px;'>Simon AI v1.2.0 • Created by Sean L. Matondo<br>Powered by Lite Wrld Gen 🌍</p></center>", unsafe_allow_html=True)