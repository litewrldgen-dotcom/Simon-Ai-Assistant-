...
File Context: {st.session_state.files_context}"""
    ...
    with st.chat_message("assistant"):
        ...
        if user_input.startswith("/img") and hf_headers:
            # HF IMAGE GEN - NOW IT ACTUALLY SHOWS THE PIC
            prompt = user_input[4:]
            ...
            try:
                ...
                ai_reply = f"Got you 🔥 📸 Bet I can, {st.session_state.memory['name']}! Here's your image of **{prompt}**"
                ai_reply += f"<br><br>📊 Image generated using HuggingFace model. Want another one?"
            except:
