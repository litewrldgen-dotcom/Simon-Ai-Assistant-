...
.title {font-size: 18px; font-weight: 700; color: #fff; display: flex; align-items: center; gap: 10px;} /* ADDED GAP SO IT DOESNT CLASH */
...
[data-testid="stChatMessage"][data-testid*="assistant"] [data-testid="stChatMessageContent"] {
    ...
    color: #e5e5e5; border-radius: 20px 20px 20px 6px; padding: 14px 18px; max-width: 70%; line-height: 1.7; /* ADDED LINE HEIGHT FOR DESCRIPTIVE TEXT */
}
...
<div class="topbar">
    <div class="logo"></div>
    <div class="title">Simon AI v1.0.1g <span style="color:#3B82F6;">✓</span> <span class="badge">LITE WRLD GEN</span></div> <!-- ADDED SPACE BEFORE BADGE -->
</div>
""", unsafe_allow_html=True)
...
def get_reply(msg):
    ...
    # DETAILED TEAM INFO LIKE YOUR SCREENSHOT
    if "sean" in msg.lower() and "kelvin" in msg.lower() or "about lite wrld gen" in msg.lower() or "team" in msg.lower():
...
Hit his WhatsApp to collab or get posted {bro}💯"""
    ...
    bro_rule = "Use 'bro' sometimes, light slang, and 2-4 emojis per reply. Be detailed and descriptive." if st.session_state.use_bro else "Be friendly, use 2-4 emojis per reply, light slang is ok. Be detailed and descriptive. Do NOT use 'bro' unless user uses it first"
    ...
    RULE 2: Talk style: {bro_rule}. Write detailed answers with full paragraphs and bullet points.
    RULE 3: NO CHARACTER LIMIT. Give complete detailed answers.
    RULE 4: NO slurs. Keep it respectful.
    RULE 5: FOR CODING: Always help. Write full code, debug, explain. Use markdown code blocks.
    ...
    try:
        res = client.chat.completions.create(
            ...
            messages=msgs,
            temperature=0.9,
            max_tokens=4000 # UNLOCKED - NO CHAR LIMIT
        )
...
for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        st.markdown(m["content"] + ('<div class="sig">Simon AI v1.0.1g • Lite Wrld Gen</div>' if m["role"]=="assistant" else ""), unsafe_allow_html=True)
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        st.markdown(ai_reply + '<div class="sig">Simon AI v1.0.1g • Lite Wrld Gen</div>', unsafe_allow_html=True)
