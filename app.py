...
<div class="topbar">
    ...
    <div>
        <div class="neon">Simon AI v2.0.6 ✓</div>
...
def get_system_prompt():
    slang_rule = "You can use light slang and emojis 💚" if st.session_state.user_uses_slang else "Be professional, warm, helpful. Minimal slang."
    return f"""
You are Simon AI v2.0.6 by Lite Wrld Gen.
ABOUT LITE WRLD GEN: We operate online worldwide. Contact: support@litewrldgen.com
PERSONALITY: {slang_rule}
RULES: 1. Rep Lite Wrld Gen first 2. Be for EVERYONE 3. Never call user bro/son unless they do first 
4. Use GROQ for chat. Use HF for images. 5. Remember name: {current_mem['name']}
"""

# ========= AI FUNCTIONS =========
def groq_chat(messages):
    if not GROQ_API_KEY: return "Add GROQ_API_KEY to secrets"
    try:
        r = requests.post("https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_API_KEY}"},
            json={"model": "llama-3.3-70b-versatile", "messages": messages}, timeout=20)
        return r.json()["choices"][0]["message"]["content"]
    except: return "Groq is down. Try again 💚"
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant":
            st.markdown('<div class="sig">Simon AI v2.0.6 • Lite Wrld Gen</div>', unsafe_allow_html=True)
...
st.markdown("<center><p style='color:#444;font-size:11px;margin-top:100px;'>Simon AI v2.0.6 • support@litewrldgen.com</p></center>", unsafe_allow_html=True)