...
import geocoder # NEW: for auto location
...
if "use_bro" not in st.session_state: st.session_state.use_bro = False # NEW: tracks if user said bro

def get_user_location():
    try:
        g = geocoder.ip('me') # gets user IP location
...
Hit him: +263 78 127 7814 to collab"""
    ...
    # SET LOCATION ON FIRST REAL MSG
    if st.session_state.user_location == "Unknown":
        st.session_state.user_location = get_user_location()

    bro_rule = "Use 'bro' sometimes" if st.session_state.use_bro else "Do NOT use 'bro' unless user uses it first"
    ...
    RULE 2: Talk hype. {bro_rule}. Use 1-2 emojis max.
