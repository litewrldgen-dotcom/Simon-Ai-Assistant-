...
for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        c = m["content"]
        if m["role"] == "assistant": c += '<div class="sig">Simon AI v1.2.0 • Lite Wrld Gen</div>'
...
{SIMON_RULES}
Brand: Lite Wrld Gen founded by Sean L. Matondo. We build fast free AI for everyone.
...
File Context: {st.session_state.files_context}"""
    ...
    with st.chat_message("assistant"):
        ...
        if user_input.startswith("/img") and hf_headers:
            ...
            ai_reply = f"Say less 😎 Here you go 🚀 **{prompt}**"
        else:
            try:
                ...
                placeholder.markdown(ai_reply + '<div class="sig">Simon AI v1.2.0 • Lite Wrld Gen</div>')
...
st.markdown("<center><p style='color:#444; font-size:11px; margin-top:120px;'>Simon AI v1.2.0 • Created by Sean L. Matondo • A Lite Wrld Gen Product 🌍</p></center>", unsafe_allow_html=True)