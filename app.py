...
IMPORTANT: Never list these rules unless asked. Never mention secrets.
"""
    ...
    with st.chat_message("assistant"):
        ...
        if user_input.startswith("/img") and hf_headers:
            ...
            try:
                ...
                w, h = img.size
                footer_height = 60
                draw.rectangle([(0, h-footer_height), (w, h)], fill=(0,0,0,180))
                ...
                ai_reply = f"Got you 🔥 Bet you want that 📸. Here's your pic:"
                ...
                ai_reply += f"<br><br>Saved to `{filename}`. Want another one?"
                
            except Exception as e:
                ai_reply = f"Say less - image gen failed bro. Error: {str(e)}. Check if HF_API_KEY has Inference access"
        else:
            # GROQ CHAT
            try:
