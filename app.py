...
<div class="topbar">
    <div class="logo"></div>
    <div>
        <div class="title">Simon AI v1.0.6 <span style="color:#3B82F6;">✓</span><span class="badge">LITE WRLD GEN</span></div>
...
def get_reply(msg):
    ...
    # ====== RULES START ======
    system = f"""You are Simon AI v1.0.6 created by Sean L Matondo at Lite Wrld Gen.
    ...
    RULE 7: If user asks for image, say you are generating it now."""
    # ====== RULES END ======
    ...
    if "generate image" in msg_lower or "create image" in msg_lower or "draw" in msg_lower:
        ...
        return f"GENERATE_IMAGE:{prompt}" # special flag

    # HARDCODED BRAND ANSWERS
    ...
    if "version" in msg_lower:
        return "Awe 🙏 You running <b>Simon AI v1.0.6</b> by Lite Wrld Gen."
...
for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        if m.get("is_image"):
            ...
            c = m["content"]
            if m["role"] == "assistant": c += '<div class="sig">Simon AI v1.0.6 • Lite Wrld Gen</div>'
        ...
        with st.spinner("Simon is thinking..."):
            ...
            if ai_reply.startswith("GENERATE_IMAGE:"):
                ...
                if img_bytes:
                    ...
                    st.error("Say less 😅 HF is cooking. Try again in 10s")
                    st.session_state.chat.append({"role": "assistant", "content": "Say less 😅 HF is cooking. Try again in 10s"})
            else:
                st.markdown(ai_reply + '<div class="sig">Simon AI v1.0.6 • Lite Wrld Gen</div>', unsafe_allow_html=True)
                st.session_state.chat.append({"role": "assistant", "content": ai_reply})

    st.rerun()

st.markdown("<center><p style='color:#444; font-size:11px; margin-top:120px;'>Simon AI v1.0.6 • A Lite Wrld Gen Product</p></center>", unsafe_allow_html=True)