...
10. If asked who built Simon, say: Created by Sean L. Matondo at Lite Wrld Gen.
...
# ========= INPUT =========
user_input = st.chat_input("Ask Simon anything... /img or 'generate pic' to make images ⚡")
...
File Context: {st.session_state.files_context}"""
    ...
    with st.chat_message("assistant"):
        ...
        # NEW: DETECT ANY IMAGE COMMAND
        lower_input = user_input.lower()
        is_image = user_input.startswith("/img") or "generate" in lower_input or "create image" in lower_input or "make a pic" in lower_input
        ...
        if is_image:
            ...
            if not hf_headers:
                ...
                # Clean prompt
                prompt = user_input.replace("/img","").replace("generate","").replace("create","").replace("a pic of","").replace("an image of","").strip()
                placeholder.markdown(f"Got you 🔥 Locked in. Generating: **{prompt}**... Wait 20s 📸")
                
                API_URL = "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell"
                res = requests.post(API_URL, headers=hf_headers, json={"inputs": prompt}, timeout=120)
                
                if res.status_code!= 200:
                    ai_reply = f"Say less - HF Error {res.status_code}: {res.text[:200]}. Check token has Inference permission"
                else:
                    try:
                        ...
                        placeholder.empty()
                        st.image(img, caption=f"Prompt: {prompt}", use_container_width=True) # THIS FORCES PIC TO SHOW
                        st.download_button("💾 Download Image", data=img.tobytes(), file_name=filename, mime="image/png")
                        ai_reply = f"Got you 🔥 Here you go {st.session_state.memory['name']}! 📸 Generated with HF."
                        
                    except Exception as e:
                        ai_reply = f"Say less - image gen failed bro. Error: {str(e)[:150]}"
        else:
            # ========= GROQ CHAT =========
...
st.markdown("<center><p style='color:#444; font-size:11px; margin-top:120px;'>Simon AI v1.2.0 • Powered by Lite Wrld Gen<br>Groq + HF 🌍</p></center>", unsafe_allow_html=True)