...
section[data-testid="stSidebar"] {
    ...
    border-right: 1px solid rgba(0,255,100,0.3);
}

/* CHAT BUBBLES */
[data-testid="stChatMessage"][data-testid*="user"] [data-testid="stChatMessageContent"] {
    background: linear-gradient(135deg, #00ff64, #00cc4f);
    color: #000; border-radius: 20px 20px 6px 20px; padding: 14px 18px; max-width: 80%; font-weight: 500;
}
[data-testid="stChatMessage"][data-testid*="assistant"] [data-testid="stChatMessageContent"] {
    background: rgba(30,30,40,0.7); backdrop-filter: blur(20px); border: 1px solid rgba(0,255,100,0.2);
    color: #e5e5e5; border-radius: 20px 20px 20px 6px; padding: 14px 18px; max-width: 80%;
}

/* FIXED INPUT + PLACEHOLDER */
.stChatInput {position: fixed!important; bottom: 0!important; left: 0!important; right: 0!important;
background: rgba(10,10,15,0.95); backdrop-filter: blur(30px); border-top: 1px solid rgba(0,255,100,0.3); padding: 16px!important;}
input::placeholder {color: #00ff64!important; opacity: 0.8!important;}

/* TOPBAR */
.topbar {position: fixed; top: 0; left: 0; right: 0; height: 70px; background: rgba(10,10,15,0.95);
backdrop-filter: blur(30px); border-bottom: 1px solid rgba(0,255,100,0.3); display: flex; align-items: center; padding: 0 24px; z-index: 999;}
.neon {color: #00ff64; text-shadow: 0 0 10px #00ff64;}
.sig {font-size: 10px; color: #00ff64; margin-top: 6px; opacity: 0.7;}
</style>
...
with st.sidebar:
    ...
    st.progress(min(len(current_mem['chats'])/20, 1.0))
    st.caption(f"{len(str(current_mem))//1024}KB / 1000MB")
...
<div class="topbar">
    ...
    <div>
        <div style="font-size:18px;font-weight:700;">Simon AI v2.0.1 <span class="neon">✓</span></div>
...
SIMON_SYSTEM = f"""
You are Simon AI v2.0.1. Powered by Lite Wrld Gen.
PERSONALITY: Daddy's Big Son. Friendly, helpful, use slang but not too much. Use emojis 💚⚡
RULES:
...
# ========= 11. AI FUNCTIONS - BUG FIXED =========
def groq_chat(messages):
    if not groq_key: return "Say less bro - GROQ_API_KEY missing in secrets"
    url = "https://api.groq.com/openai/v1/chat/completions"
    headers = {"Authorization": f"Bearer {groq_key}", "Content-Type": "application/json"}
    payload = {"model": "llama-3.3-70b-versatile", "messages": messages, "stream": False}
    try:
        r = requests.post(url, headers=headers, json=payload, timeout=30)
        return r.json()["choices"][0]["message"]["content"]
    except Exception as e:
        return f"Say less bro Groq lagging: {str(e)[:50]} ⚡"

def hf_image_gen(prompt):
    if not hf_headers: return None, "HF_API_KEY missing in secrets"
    API_URL = "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell"
    try:
        r = requests.post(API_URL, headers=hf_headers, json={"inputs": prompt}, timeout=90)
        if r.status_code!= 200:
            err = r.json().get("error", "Unknown")
            return None, f"Model loading... try again in 20s. {err}"
        ...
        return None, "Internet/HF connection failed. Try again son"
    except Exception as e:
        return None, f"Image error: {str(e)[:80]}"
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant":
            st.markdown('<div class="sig">Simon AI v2.0.1 • Powered by Lite Wrld Gen</div>', unsafe_allow_html=True)

# ========= 13. INPUT - FIXED PLACEHOLDER =========
user_input = st.chat_input("Ask Simon Anything ⚡") # CHANGED THIS
...
# ========= 14. FOOTER =========
st.markdown("<center><p style='color:#444;font-size:11px;margin-top:100px;'>Simon AI v2.0.1 • Powered by Lite Wrld Gen<br>Groq Llama 3.3 70B + HF FLUX ⚡</p></center>", unsafe_allow_html=True)