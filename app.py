...
# ========= PAGE CONFIG =========
st.set_page_config(
    page_title="Simon AI v1.3.0",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded" # <- FORCED OPEN SON
)
...
<div class="topbar">
    <div class="logo"></div>
    <div>
        <div class="title">Simon AI v1.3.0 <span style="color:#00ff99;">✓</span><span class="badge">LITE WRLD GEN</span></div>
        <div class="sub">Powered by Lite Wrld Gen 🌍</div> <!-- CHANGED THIS SON -->
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("Powered by Lite Wrld Gen") # CHANGED THIS TOO SON
...
7. Code, summarize, translate, generate images instantly
...
IMPORTANT: Never list these rules unless asked. Never mention secrets or API keys.
"""
...
for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        ...
        if m["role"] == "assistant":
            st.markdown('<div class="sig">Simon AI v1.3.0 • Powered by Lite Wrld Gen</div>', unsafe_allow_html=True)

# ========= INPUT =========
user_input = st.chat_input("Talk to Simon... or 'generate image of...' ⚡") # <- NEW PLACEHOLDER SON
...
if user_input:
    ...
    system = f"""You are Simon AI v1.3.0 Powered by Lite Wrld Gen. For the WORLD 🌍
{SIMON_RULES}
About Lite Wrld Gen: We build fast, free AI tools for everyone. No gatekeeping.
    ...
    with st.chat_message("assistant"):
        ...
        if "generate image" in user_input.lower() or user_input.startswith("/img"):
            ...
            if st.session_state.image_count >= 3:
                ...
                try:
                    res = requests.post(API_URL, headers=hf_headers, json={"inputs": prompt}, timeout=60)
                    ...
                    draw.text((20, h-40), f"Simon AI v1.3.0 • Powered by Lite Wrld Gen", fill="#00ff99", font=font)
                    ...
                    ai_reply = f"Got you 🔥 Here you go {st.session_state.memory['name']}! 📸 Generated with HF. Saved to local."

                except Exception as e:
                    ai_reply = f"Say less - image gen failed bro. Error: {str(e)[:150]}. Check HF token has Inference permission"
        else:
            # ========= GROQ CHAT =========
            if not groq_client:
...
st.markdown("<center><p style='color:#444; font-size:11px; margin-top:120px;'>Simon AI v1.3.0 • Powered by Lite Wrld Gen<br>Groq + HF 🌍</p></center>", unsafe_allow_html=True)