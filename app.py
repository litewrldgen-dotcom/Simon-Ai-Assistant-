...
File Context: {st.session_state.files_context}"""
    ...
    with st.chat_message("assistant"):
        ...
        if user_input.startswith("/img") and hf_headers:
            # HF IMAGE GEN - ACTUALLY RENDERS NOW
            prompt = user_input[4:]
            API_URL = "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell"
            res = requests.post(API_URL, headers=hf_headers, json={"inputs": prompt})
            try:
                img = Image.open(BytesIO(res.content))
                ai_reply = f"Got you 🔥 Bet you want that cute pup 😍. Here's a dog pic I generated just for you 📸<br><br>"
                placeholder.markdown(ai_reply, unsafe_allow_html=True)
                st.image(img, caption=f"[Generated Dog Image]", use_container_width=True)
                ai_reply += f"<br>(If image doesn't show up, try opening on mobile)<br><br>If you want more variations just say the breed name. No cap! 🐶"
            except:
