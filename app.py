...
img {border-radius: 16px; margin-top: 10px; max-width: 100%; border: 1px solid rgba(124,58,237,0.3);}
...
def get_news():
    try:
        # Using NewsAPI - you can add key to secrets later
...
def generate_image(prompt):
    clean_prompt = prompt.replace("draw ", "").replace("generate ", "").replace("create ", "")
    timestamp = int(time.time())
    url = f"https://image.pollinations.ai/prompt/{requests.utils.quote(clean_prompt)}?width=1024&height=1024&model=flux&nologo=true&seed={timestamp}"
    return url
...
def get_reply(msg):
    ...
    if "where am i" in msg.lower() or "my location" in msg.lower():
        return f"Awe {st.session_state.user_name} 😏 You're in {st.session_state.user_location} bro. No cap.", None
    ...
    image_keywords = ["draw", "generate image", "create image", "picture of", "make me"]
    if any(word in msg.lower() for word in image_keywords):
        img_url = generate_image(msg)
        reply = f"Say less {st.session_state.user_name} 🔥 Cooking this up for you right now!"
        return reply, img_url
    
    system = f"""You are Simon AI v1.0.0 by Lite Wrld Gen. The user's name is {st.session_state.user_name}. Current date: {datetime.now().strftime('%Y-%m-%d')}. User location: {st.session_state.user_location}.
    ...
    RULE 10: You have access to real-time news and can generate images."""

    msgs = [{"role": "system", "content": system}]
    for m in st.session_state.chat: 
        if "image" in m: m_copy = {"role": m["role"], "content": m["content"]}
        else: m_copy = m
        msgs.append(m_copy)
...
for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        ...
        if "image" in m and m["image"]:
            st.image(m["image"], caption="Generated by Simon 🔥")
            img_data = requests.get(m["image"]).content
            st.download_button("💾 Save Image", img_data, file_name=f"simon_gen_{int(time.time())}.png", mime="image/png", key=f"save_{m.get('id', time.time())}")

user_input = st.chat_input("Ask Simon anything... ✨")

if user_input:
    st.session_state.chat.append({"role": "user", "content": user_input, "id": time.time()})
    with st.chat_message("assistant"):
        typing_placeholder = st.empty()
        typing_placeholder.markdown('<div class="typing">Simon is cooking... 🤔</div>', unsafe_allow_html=True)
        time.sleep(0.5)
        ...
        if img_url:
            st.image(img_url, caption="Generated by Simon 🔥")
            img_data = requests.get(img_url).content
            st.download_button("💾 Save Image", img_data, file_name=f"simon_gen_{int(time.time())}.png", mime="image/png", key=f"save_{time.time()}")
            st.session_state.chat.append({"role": "assistant", "content": ai_reply, "image": img_url, "id": time.time()})
        else:
            st.session_state.chat.append({"role": "assistant", "content": ai_reply, "id": time.time()})
    st.rerun()