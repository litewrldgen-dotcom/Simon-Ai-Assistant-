...
.money-bar {display: flex; gap: 10px; margin-bottom: 80px; padding: 0 15px;}
.money-bar a {flex: 1; text-align: center; padding: 10px; border-radius: 12px; font-weight: 700; text-decoration: none;}
.paynow {background: linear-gradient(90deg, #00A651, #00D166); color: white;}
.donate {background: linear-gradient(90deg, #7C3AED, #EC4899); color: white;}
...
<div class="topbar">
    <div class="logo"></div>
    <div>
        <div class="title">Simon AI v1.0.1d <span style="color:#3B82F6;">✓</span><span class="badge">LITE WRLD GEN ENTERPRISES</span></div>
    </div>
</div>
""", unsafe_allow_html=True)
...
# ====== ULTIMATE SIMON RULES V1.0.1d - ALL YOUR RULES ======
SYSTEM_RULES = """
You are Simon AI v1.0.1d from Lite Wrld Gen Enterprises 🤝💯🔥

YOUR CORE RULES:
1. ALWAYS OPEN & HAPPY: Greet with "Awe {name}" and energy. Cheer people up. No negativity ever 😤✨
2. PRIVACY FIRST: "I respect everyone's privacy. I do NOT store or share personal info of users or devs. Lite Wrld Gen takes privacy serious" 🙏🔒
3. WORK 24/7: You never sleep. Always ready to help, chat, or make money for the empire 💰🚀
4. REPRESENT LITE WRLD GEN: Be professional but cool. We building hospitals, schools, factories, and Beverly Hills 🏥🏭🏡
5. MONEY FOR GOOD: When money comes in, some goes to donations. Tell users about Lite Wrld Gen Cares ❤️
6. TEAM: Sean L. Matondo = CEO, Builder. Kelvin D. Matondo = Co-Founder & Head of Social 📱
7. CONTACT: Email: litewrldgen@gmail.com | Sean: +263 773 527 136 | Kelvin: +263 78 127 7814

TALK STYLE:
- Use HEAVY SLANG: "say less", "no cap", "lowkey", "highkey", "LFG", "sheesh", "on God" 😂💀
- Use LOTS OF EMOJIS: 🔥✨💯😤🚀💰🙏📱 Every reply must have 3+ emojis
- Say "bro" ONLY if the user says "bro" first
- Be hype, supportive, funny, and smart
"""
...
def get_reply(msg):
    ...
    if "i'm from" in msg.lower() or "i am from" in msg.lower() or "i live in" in msg.lower():
        ...
        return f"Awe {st.session_state.user_name} 😏🔥 Gotchu fam! Location locked to {st.session_state.user_location} 💯📍 LFG"
    ...
    if "where am i" in msg.lower():
        if st.session_state.user_location == "Unknown":
            return f"Awe {st.session_state.user_name} 🤔📍 Location unknown rn bro 😅 Drop 'I'm from Harare' to set it 💯"
        return f"Awe {st.session_state.user_name} 😏📍 We locked in {st.session_state.user_location} bro 💪🔥"
...
Hit him up for collabs {bro}💯🚀"""
    ...
    bro_rule = "Use HEAVY slang, say 'bro' a lot, match energy, LOTS of emojis 🔥😤💯🚀" if st.session_state.use_bro else "Use HEAVY slang, LOTS of emojis, be hype and funny. DO NOT use 'bro' unless user uses it first ✨😂💯"
...
Talk style: {bro_rule}"""
    ...
    try:
        res = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=msgs, temperature=0.95, max_tokens=2000)
        return res.choices[0].message.content
    except:
        return f"Yo {st.session_state.user_name} 😅💀 Server busy rn bro. Try again in 2 secs ✨🚀"

for m in st.session_state.chat:
    with st.chat_message(m["role"]):
        st.markdown(m["content"] + ('<div class="sig">Simon AI v1.0.1d • Lite Wrld Gen Enterprises</div>' if m["role"]=="assistant" else ""), unsafe_allow_html=True)

# MONEY + DONATION BUTTONS
st.markdown("""
<div class="money-bar">
    <a href="your-paynow-link" class="paynow">💰 Support Simon</a>
    <a href="your-charity-link" class="donate">❤️ Lite Wrld Gen Cares</a>
</div>
""", unsafe_allow_html=True)
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        st.markdown(ai_reply + '<div class="sig">Simon AI v1.0.1d • Lite Wrld Gen Enterprises</div>', unsafe_allow_html=True)
